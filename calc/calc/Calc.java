...
public class Calc {
    ...
    private static String opn(String sIn) throws CalcException {
        ...
        if(sIn.length() == 0) {
            throw new CalcException("Длина входной строки 0");
        }

        if(checkSeq(sIn)) {
            throw new CalcException("Неверная скобочная последовательность");
        }
        ...
        for (int i = 0; i < sIn.length(); i++) {
            ...
            if (isOp(cIn)) {
                ...
                sbStack.setLength(sbStack.length()-1);
            } else if (cIn >= 48 && cIn <= 57) {
                // Если символ не оператор - добавляем в выходную последовательность
                sbOut.append(cIn);
            } else {
                // Если символ не оператор не скобка и не число
                throw new CalcException("Неопознанный символ");
            }
        }
    ...
    private static Boolean checkSeq(String inS) {
    ...
    public static double calculate(String sIn) throws CalcException {
        ...
        if (!st.hasMoreTokens()) {
            throw new CalcException("Пустая строка или строка с одними символами табуляции");
        }
            ...
            try {
                ...
                if (1 == sTmp.length() && isOp(sTmp.charAt(0))) {
                    ...
                    try {
                        ...
                        throw new CalcException("Неверная запись числа");
                    }
                }
